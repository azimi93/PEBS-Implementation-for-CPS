% ------------------------------------------------------------------------
% Potential energy boundary surface (PEBS) method used in:
% Hybrid Game-Theoretic Security Assessment of Cyber-Physical Power Systems
% Using Partial-Information Multi-Agent Reinforcement Learning 
% ------------------------------------------------------------------------
% Author:       [Z.Azimi, A.Afshar]
% Affiliation:  [Amirkabir University of technology]
% DOI:          [10.1016/j.segan.2025.101727]
%
clear; close all;

%% Settings
sys_case=9; % IEEE system
fault_line=4; % Line number of the faulted line 4
fault_frto_bus=1; % From or To bus where the fault is applied 1 %2
...
t_fault=0; % time that the fault is applied %%0.1 jaleb mishe
...
scatter(theta_u(plot_axis(1)),0,100,'w','filled')
scatter(x_fault(plot_axis(1),PEBS_idx),0,100,'r','filled')
plot(x_sim(plot_axis(1),:),x_sim(plot_axis(2),:),'b','LineWidth',2);
streamslice(x1_plot_ij,x2_plot_ij,dx1_plot_post,dx2_plot_post);
axis(plot_rng); 
set(gca,'FontSize',15,'FontName','Times New Roman'); xlabel('\delta (rad)'); ylabel('\omega (rad/s)'); title('Post-contingency Dynamics'); legend('u.e.p. (PEBS)');% legend('u.e.p. (BCU)','u.e.p. (PEBS)')

% Plot Energy function
figure; hold all; grid on; box on;
scatter(theta_u(plot_axis(1)),0,100,'w','filled')
scatter(x_fault(plot_axis(1),PEBS_idx),0,100,'w','filled')
streamslice(x1_plot_ij,x2_plot_ij,dx1_plot_post,dx2_plot_post);
...
% Plot the equilibrium points (if relevant)
%scatter3(theta_u(plot_axis(1)), 0, 0, 100, 'r', 'filled');  % UEP (BCU)
scatter3(x_fault(plot_axis(1), PEBS_idx), 0, 0, 100, 'b', 'filled');  % UEP (PEBS)

% Create a mesh grid for the 3D plot
[x1_plot_ij, x2_plot_ij] = meshgrid(linspace(plot_rng(1), plot_rng(2), 30), linspace(plot_rng(3), plot_rng(4), 30));

% Plot the energy function as a surface plot
mesh(x1_plot_ij, x2_plot_ij, V_plot);  % Use 'surf' for a smoother surface

% Set the axis labels and titles
xlabel('\delta (rad)'); ylabel('\omega (rad/s)'); zlabel('V(\delta, \omega)');
title('3D Energy Function Plot');
set(gca, 'FontSize', 15, 'FontName', 'Times New Roman');

% Optional: Add a color bar to indicate energy levels
colorbar;
view(3)
%%%%%%%%%%%%%%%
